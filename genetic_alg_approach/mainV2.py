...
original_image = Image.open("./input/image.png").convert("L")
original_height, original_width  = original_image.size
cv2.imshow("Original", np.array(original_image))
cv2.imwrite("./input/original.png",np.array(original_image))
...
def whole_pipeline():
    ...
    imageio.mimsave("./output/output_gif.gif", save_gif)
    cv2.imwrite("./output/output_best.jpg", open_cv_image) 
